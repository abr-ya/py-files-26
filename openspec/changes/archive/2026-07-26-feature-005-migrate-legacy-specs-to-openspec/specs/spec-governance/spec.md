...
- **AND** the contributor is not directed to `specs/` as current authority

### Requirement: Legacy Specs Marker

The project MUST mark the root `specs/` tree as legacy reference material after migration.

#### Scenario: Reader opens legacy specs

- **GIVEN** a reader opens the legacy `specs/` directory
